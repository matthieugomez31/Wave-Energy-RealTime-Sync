...
4.  **Data Return:** The ESP32 packages the synchronized tuple `(Timestamp, X, Y, V, I)` and sends it back to the PC for logging[cite: 48, 49, 50, 51].
...
Hardcoding a pixel-to-mm ratio is error-prone due to camera vibrations. I implemented a **per-frame calibration** system using a 3-marker setup[cite: 67, 68]:
* **Reference Markers (ID 10 & 11):** Placed at a fixed known distance.
* **Algorithm:** The script recalculates the scale factor ($mm/px$) for every single frame based on the distance between Ref 1 and Ref 2[cite: 74, 75].
* **Result:** Robust measurement accuracy ($\pm$ 0.5 mm) even if the camera focus or position shifts slightly[cite: 83].

### 2. Electrical Hardware Setup
* **Microcontroller:** ESP32-DevKitC (Dual-core, High speed)[cite: 88].
* **Sensor:** INA219 (High-Side DC Current/Voltage) over I2C[cite: 89, 90].
...
2.  **Software Sequence:**
    * Close any open Serial Monitors (Arduino IDE) to free the COM port[cite: 136].
    * Run the script: `python calibrated_logger.py`
    * Wait 4 seconds for buffer cleaning[cite: 140].
3.  **Output:** Data is saved to `log_experience_YYYYMMDD.csv` for MATLAB/Excel analysis[cite: 146].
